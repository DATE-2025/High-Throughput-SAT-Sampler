...
import sympy
from sympy import Or, And, Not, Equivalent
...
def transformer(cnf):
    ...
    uncommon_elements = list(hardcoded_inputs_list - set(output_list))
...
def generate_pytorch_model(inputs, outputs, variables, clause_gate, clause_output):
    ...
    for i in range(len(clause_output)):
        if clause_gate[i] and not (clause_gate[i] == True or  clause_gate[i] == False) and not (clause_output[i] == 0):
